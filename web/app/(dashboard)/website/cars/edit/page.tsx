...
function CarFormContent() {
    ...
    async function fetchCar() {
        const { data } = await supabase.from('website_cars').select('*').eq('id', id).single()
        if (data) {
            setForm({
                slug: data.slug || '',
                brand: data.brand || '',
    ...
    function removeFeature(index: number) {
        setForm(prev => ({ ...prev, features: prev.features.filter((_: string, i: number) => i !== index) }))
    }

    function removeFeatureFr(index: number) {
        setForm(prev => ({ ...prev, features_fr: prev.features_fr.filter((_: string, i: number) => i !== index) }))
    }
    ...
    function removeColor(index: number) {
        setForm(prev => ({ ...prev, colors: prev.colors.filter((_: string, i: number) => i !== index) }))
    }

    function removeColorFr(index: number) {
        setForm(prev => ({ ...prev, colors_fr: prev.colors_fr.filter((_: string, i: number) => i !== index) }))
    }
    ...
    async function handleSubmit(e: React.FormEvent) {
        ...
        try {
            ...
            if (isEdit) {
                await supabase.from('website_cars').update(payload).eq('id', id)
            } else {
                ...
                <section className="bg-white dark:bg-slate-800/50 rounded-2xl border border-gray-100 dark:border-slate-700/50 p-6 space-y-5">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <div className="flex flex-wrap gap-2 mb-3">
                                {form.features_fr.map((f: string, i: number) => (
                ...
                <section className="bg-white dark:bg-slate-800/50 rounded-2xl border border-gray-100 dark:border-slate-700/50 p-6 space-y-5">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <div className="flex flex-wrap gap-2 mb-3">
                                {form.colors_fr.map((c: string, i: number) => (
